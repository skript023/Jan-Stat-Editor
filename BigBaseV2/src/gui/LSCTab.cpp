        ...
        g_fiber_pool->queue_job([]
            {
                if (PED::IS_PED_IN_ANY_VEHICLE(PLAYER::PLAYER_PED_ID(), FALSE))
                {
                    if (player_vehicle != PED::GET_VEHICLE_PED_IS_IN(PLAYER::PLAYER_PED_ID(), FALSE))
                    {
                        ...
                        VEHICLE::_GET_VEHICLE_INTERIOR_COLOUR(player_vehicle, &interior_color);
                        VEHICLE::_GET_VEHICLE_DASHBOARD_COLOUR(player_vehicle, &dashboard_color);
                        ...
                        for (int i = 0; i < mod_display_names[selected_slot].size(); i++)
                        {
                            ...
                            if (Imgui::Selectable(mod_display_names[selected_slot][i].c_str(), i == owned_mods[selected_slot]))
                            {
                                g_fiber_pool->queue_job([i]
                                    {
                                        network_helper::request_control(player_vehicle);
                    ...
                    case 6: //Interior Color
                    {
                        for (int i = 0; i < 74; i++)
                        {
                            if (Imgui::Selectable(classic_names[i], color_check == classic_ids[i]))
                            {
                                ...
                                g_fiber_pool->queue_job([]
                                    {
                                        VEHICLE::SET_VEHICLE_INTERIOR_COLOUR(player_vehicle, interior_color);
                                    });
                            }
                        }
                        break;
                    ...
                    case 7: //Dashboard Color
                    {
                        for (int i = 0; i < 74; i++)
                        {
                            if (Imgui::Selectable(classic_names[i], color_check == classic_ids[i]))
                            {
                                ...
                                g_fiber_pool->queue_job([]
                                    {
                                        VEHICLE::SET_VEHICLE_DASHBOARD_COLOUR(player_vehicle, dashboard_color);
                                    });
                            }
                        }
                        break;
